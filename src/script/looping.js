const toppings = ['Mushrooms ', 'Tomatoes', 'Eggs', 'Chili', 'Lettuce', 'Avocado', 'Chiles', 'Bacon', 'Pickles', 'Onions', 'Cheese'];
    ...
    function logItemsInArray(item, index, originalArray) {
        ...
        previtem ? console.log('prev :' + previtem) : null;
        // log the next item if there is one
        nextitem ? console.log('next :' + nextitem) : null;
        // if its the last item in the array, say good bye
        index === originalArray.length -1 ? console.log('Goodbye') : console.log(`Getting the next item Index:${index} length:${originalArray.length}`);
  ...
  function tallyNumbers(prvNumber, currentNumber) {
    ...
    console.log('----------------------');
    // return the current tally PLUS the amount of this order return prvNumber + currentNumber;
  }
